...
export class RoomService {
  ...
  async updateStatus(
    ...
    this.logger.debug(`Room ${roomNumber} status updated to ${String(status)}.`);
    return room;
  }
  ...
  async handleCheckOut(roomNumber: number): Promise<RoomEntity> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to publish checkout event for room ${roomNumber}: ${err instanceof Error ? err.message : String(err)}`,
  ...
  async resetRoomStatus(
    ...
    this.logger.log(
      `Attempting to reset or create room ${roomNumber} to ${String(status)}`
    );
