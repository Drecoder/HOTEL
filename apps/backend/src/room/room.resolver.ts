...
export class RoomResolver {
  ...
  async resetRoom(
    ...
    // Map RoomEntity to RoomType, converting null to undefined for expectedCheckout
    return {
      ...room,
      expectedCheckout: room.expectedCheckout === null ? undefined : room.expectedCheckout,
    };
  }
  // ----------------------------
  // QUERIES
  ...
  async checkOut(
    ...
    const room = await this.roomService.handleCheckOut(roomNumber);
    return {
      ...room,
      expectedCheckout: room.expectedCheckout === null ? undefined : room.expectedCheckout,
    };
  ...
  async cleanRoom(
    ...
    const room = await this.roomService.handleCleanRoom(roomNumber);
    return {
      ...room,
      expectedCheckout: room.expectedCheckout === null ? undefined : room.expectedCheckout,
    };
